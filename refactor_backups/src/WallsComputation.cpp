...
void WallsComputation::generateSpiralInsets(SliceLayerPart* part, coord_t line_width_0, coord_t wall_0_inset, bool recompute_outline_based_on_outer_wall, coord_t layer_z)
{
    ...
    // 添加调试信息，显示当前处理的多边形数量
    CURA_DEBUG(WALL_COMPUTATION, "generateSpiralInsets: only_spiralize_out_surface={}, 多边形数量={}",
    ...
    {
        // 输出功能启用信息和原始状态
        CURA_DEBUG(WALL_COMPUTATION, "=== only_spiralize_out_surface功能启用 ===");
        CURA_DEBUG(WALL_COMPUTATION, "原始多边形数量: {}", spiral_outline.size());
        ...
        for (size_t i = 0; i < spiral_outline.size(); ++i)
        {
            ...
            // 输出每个多边形的详细信息用于调试
            CURA_DEBUG(WALL_COMPUTATION, "多边形[{}]: 面积={:.2f}mm², 顶点数={}",
        ...
        // 输出筛选结果的详细信息
        CURA_DEBUG(WALL_COMPUTATION, "保留最外层多边形[{}]: 面积={:.2f}mm², 顶点数={}",
                    max_area_index, INT2MM2(max_area), outer_polygon.size());
        CURA_DEBUG(WALL_COMPUTATION, "过滤后多边形数量: {}", spiral_outline.size());
    ...
    if (reinforce_layers > 0 && (layer_nr_ - initial_bottom_layers) < reinforce_layers)
    {
        ...
        for (size_t i = 0; i < actual_contour_count; i++)
        {
            if (i == 0)
            {
                ...
                CURA_DEBUG(WALL_COMPUTATION, "【螺旋加强】最内圈（可变），偏移{}μm，宽度{}μm（{:.2f}倍wall_0）",
            ...
            else
            {
                ...
                CURA_DEBUG(WALL_COMPUTATION, "【螺旋加强】外圈{}（固定），偏移{}μm，宽度{}μm（1.0倍wall_0）",
...
Polygon WallsComputation::insertZSeamInterpolationPointsForSpiral(const Polygon& polygon, coord_t layer_z)
{
    ...
    if (mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::RAFT)
    {
        ...
        CURA_DEBUG(WALL_COMPUTATION, "螺旋模式插值Raft计算: 层Z={:.2f}mm, Raft厚度={:.2f}mm, Raft间隙={:.2f}mm, 模型Z={:.2f}mm",
                     INT2MM(layer_z), INT2MM(raft_total_thickness), INT2MM(raft_airgap), INT2MM(model_z));
    }
    else
    {
        CURA_DEBUG(WALL_COMPUTATION, "螺旋模式插值无Raft结构，模型Z={:.2f}mm（等于层Z）", INT2MM(model_z));
    }

    CURA_DEBUG(WALL_COMPUTATION, "螺旋模式插值处理: 层Z={:.2f}mm, 模型Z={:.2f}mm, 多边形顶点数={}", INT2MM(layer_z), INT2MM(model_z), polygon.size());
    ...
    {
        CURA_DEBUG(WALL_COMPUTATION, "螺旋模式插值计算失败，返回原多边形");
        return polygon;
    }
    ...
    if (points.size() < 3)
    {
        CURA_DEBUG(WALL_COMPUTATION, "螺旋模式多边形顶点数不足，返回原多边形");
        return polygon;
    }
    ...
    if (need_insert_point)
    {
        ...
        CURA_DEBUG(WALL_COMPUTATION, "螺旋模式在索引{}插入新点: ({:.2f}, {:.2f})",
                    insert_idx, INT2MM(closest_point_on_segment.X), INT2MM(closest_point_on_segment.Y));
        CURA_DEBUG(WALL_COMPUTATION, "螺旋模式多边形顶点数: {} -> {}", polygon.size(), result_polygon.size());
