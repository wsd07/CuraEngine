...
int Mesh::getFaceIdxWithPoints(int idx0, int idx1, int notFaceIdx, int notFaceVertexIdx) const
{
    ...
    {
        CURA_DEBUG(MESH_PROCESSING, "Couldn't find face connected to face {}", notFaceIdx);
    ...
    {
        CURA_DEBUG(MESH_PROCESSING, "Edge with uneven number of faces connecting it!({})\n", candidateFaces.size() + 1);
        if (! has_disconnected_faces)
        {
            spdlog::warn("Mesh has disconnected faces!");
        }
    ...
    if (n0.vSize() <= 0)
    {
        CURA_DEBUG(MESH_PROCESSING, "Face {} has zero area!", notFaceIdx);
    }
    ...
    for (int candidateFace : candidateFaces)
    {
        ...
        if (angle == 0)
        {
            CURA_DEBUG(MESH_PROCESSING, "Overlapping faces: face {} and face {}.", notFaceIdx, candidateFace);
