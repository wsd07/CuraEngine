...
unsigned int FffGcodeWriter::findSpiralizedLayerSeamVertexIndex(const SliceDataStorage& storage, const SliceMeshStorage& mesh, const int layer_nr, const int last_layer_nr)
{
    ...
    if (layer_nr >= 0 && layer_nr < static_cast<LayerIndex>(mesh.layers.size()))
    {
        ...
        CURA_DEBUG(GCODE_GENERATION, "螺旋模式接缝点：第{}层，使用正确的printZ={:.2f}mm（支持可变层厚）",
    ...
    if (mesh_group_settings.get<EPlatformAdhesion>("adhesion_type") == EPlatformAdhesion::RAFT)
    {
        ...
        CURA_DEBUG(GCODE_GENERATION, "Raft计算: 层Z={:.2f}mm, Raft厚度={:.2f}mm, Raft间隙={:.2f}mm, 模型Z={:.2f}mm",
                     INT2MM(layer_z), INT2MM(raft_total_thickness), INT2MM(raft_airgap), INT2MM(model_z));
    }
    else
    {
        CURA_DEBUG(GCODE_GENERATION, "无Raft结构，模型Z={:.2f}mm（等于层Z）", INT2MM(model_z));
    }
...
{
    CURA_DEBUG(GCODE_GENERATION, "GcodeWriter processing layer {} of {}", layer_nr, total_layers);
    ...
    const SliceMeshStorage& mesh) const
{
    ...
    for (size_t wall_idx = 0; wall_idx < part.spiral_wall.size(); wall_idx++)
    {
        ...
        if (is_reinforcement_contour)
        {
            ...
            if (wall_idx < part.spiral_wall_width.size())
            {
                ...
                CURA_DEBUG(GCODE_GENERATION, "【螺旋加强线宽】第{}层，加强圈{}，使用存储的线宽：{}μm",
                             layer_nr, wall_idx, reinforcement_line_width);
            }
            else
            {
                CURA_DEBUG(GCODE_GENERATION, "【螺旋加强线宽】第{}层，加强圈{}，索引超出范围，使用默认线宽：{}μm",
                            layer_nr, wall_idx, reinforcement_line_width);
            }
            ...
            CURA_DEBUG(GCODE_GENERATION, "【螺旋加强起点优化】第{}层，加强圈{}，原起点索引：{}，优化后起点索引：{}，距离：{:.2f}mm",
    ...
    if (is_top_layer)
    {
        ...
        {
            CURA_DEBUG(GCODE_GENERATION, "【螺旋结束】第{}层是最高层，添加螺旋结束墙", layer_nr);
    ...
    SliceLayerPart& part) const
{
    ...
    if (Application::getInstance().current_slice_->scene.current_mesh_group->settings.get<bool>("magic_spiralize"))
    {
        ...
        if (!range_string.empty())
        {
            ...
            if (!magic_spiralize_range.isEmpty())
            {
                ...
                {
                    layer_z = mesh.layers[layer_nr].printZ;
                    CURA_DEBUG(GCODE_GENERATION, "螺旋范围控制：第{}层，使用正确的printZ={:.2f}mm（支持可变层厚）",
                                 layer_nr, INT2MM(layer_z));
                }
    ...
    else
    {
        ...
        if (layer_nr >= 0 && layer_nr < static_cast<LayerIndex>(mesh.layers.size()))
        {
            ...
            CURA_DEBUG(GCODE_GENERATION, "外轮廓层Z坐标计算: 层号={}, 使用正确的printZ={:.2f}mm（支持可变层厚）",
                         layer_nr, INT2MM(layer_z));
        }
        else
        {
            ...
            layer_z = 0;
            CURA_DEBUG(GCODE_GENERATION, "Raft层(层号={}), 外轮廓不使用自定义接缝点", layer_nr);
        }
...
void FffGcodeWriter::finalize()
{
    ...
    {
        CURA_INFO("Gcode header after slicing: {}", prefix);
    }
